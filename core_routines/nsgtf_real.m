function [c,Ls] = nsgtf_real(f,g,shift,M,SL)

% NSGTF_REAL.M - Gino Velasco, Nicki Holighaus 01.03.12
%
% [c,Ls] = nsgtf_real(f,g,shift,M,SL)
% 
...
%                         matrix
%           SL		: [Internal parameter] Do not use this if you run
%			  the routine manually.
%
% Output:
...
for ii = 1:N
    ...
    win_range = mod(timepos(ii)+(-floor(Lg(ii)/2):ceil(Lg(ii)/2)-1),Ls+fill)+1;
    
    if M(ii) < Lg(ii) % if the number of frequency channels is too small, aliasing is introduced
        col = ceil(Lg(ii)/M(ii));
